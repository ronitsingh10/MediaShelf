...
const BookTracker2 = ({ username, mediaType = "books", media }) => {
  ...
  const statuses = Object.entries(statusMap[mediaType] || statusMap.books).map(
    ([id, status]) => ({
      id: parseInt(id),
      name: status.label,
      icon: status.icon,
      ...
      count: categorized[mediaType]?.[id]?.length || 0, // Placeholder count for demo purposes
    })
  );
  ...
  return (
    <div className="w-full max-w-10xl mx-auto bg-white text-slate-800 p-3 rounded-lg shadow-sm border border-slate-100">
      ...
      <div className="grid grid-cols-1 md:grid-cols-12 gap-2">
        ...
        <div className="md:col-span-5 space-y-1">
          {statuses.map((status) => {
            ...
            console.log("Button: ", status.id, status.active, buttonClasses);
        ...
        {/* Books display */}
        <div className="md:col-span-7 bg-slate-50 rounded-lg p-3 border border-slate-100">
          <div className="flex justify-between items-center mb-3">
            <h2 className="text-lg font-medium capitalize">
              {categorized[mediaType]?.[activeStatus]?.length > 0 &&
                `${categorized[mediaType][activeStatus].length} ${mediaType}`}
            </h2>
          </div>
            ...
            <div className="flex gap-3 min-w-max">
              {categorized[mediaType]?.[activeStatus]?.length > 0 ? (
                ...
                <p className="text-slate-400 text-center pt-0 py-8">
                  No {mediaType.toLowerCase()} found for this status.
                </p>
              )}
            </div>
          </div>
